...
var _ = stepmeta.MustRegister[CheckHost](stepmeta.Definition{
	...
	Ask: stepmeta.AskMetadata{
		...
		ValidationHints: []stepmeta.ValidationHint{{ErrorContains: "checkhost", Fix: "For CheckHost, use spec.checks as a YAML string array like [os, arch, swap]."}, {ErrorContains: "checks is required", Fix: "CheckHost requires spec.checks. Example: spec: {checks: [os, arch, swap]}."}, {ErrorContains: "spec.binaries", Fix: "When spec.checks includes binaries, provide spec.binaries as a non-empty YAML string array."}, {ErrorContains: "additional property os is not allowed", Fix: "Do not use spec.os for CheckHost; put named checks under spec.checks instead."}, {ErrorContains: "spec.checks.0: invalid type", Fix: "Each CheckHost spec.checks item must be a plain string such as os or arch, not an object."}},
	},
})
