...
var _ = stepmeta.MustRegister[DownloadImage](stepmeta.Definition{
	...
	Ask: stepmeta.AskMetadata{
		...
		Builders: []stepmeta.AuthoringBuilder{{
			...
			Phase:                "images",
			DefaultStepID:        "prepare-download-images",
			Summary:              "Stage offline image archives with typed image download schema.",
			RequiresCapabilities: []string{"image-staging"},
			Bindings: []stepmeta.AuthoringBinding{
				{Path: "spec.images", From: "override:images", Required: true},
				{Path: "spec.backend.engine", From: "override:backendEngine"},
				{Path: "spec.backend.engine", From: "const:go-containerregistry", Required: true},
				{Path: "spec.outputDir", From: "override:outputDir", Semantic: "image-output-dir"},
				{Path: "spec.outputDir", From: "program:artifacts.imageOutputDir", Semantic: "image-output-dir"},
				{Path: "spec.outputDir", From: "derive:artifacts.imageOutputDir", Semantic: "image-output-dir", Required: true},
			},
		}},
...
var _ = stepmeta.MustRegister[LoadImage](stepmeta.Definition{
	...
	Ask: stepmeta.AskMetadata{
		...
		Builders: []stepmeta.AuthoringBuilder{{
			ID:                   "apply.load-image",
			Phase:                "load-images",
			DefaultStepID:        "apply-load-images",
			Summary:              "Load prepared image archives into the local runtime.",
			RequiresCapabilities: []string{"image-staging"},
			Bindings: []stepmeta.AuthoringBinding{
				{Path: "spec.images", From: "override:images"},
				{Path: "spec.images", From: "program:artifacts.images", Required: true},
				{Path: "spec.sourceDir", From: "override:sourceDir", Semantic: "image-output-dir"},
				{Path: "spec.sourceDir", From: "program:artifacts.imageOutputDir", Semantic: "image-output-dir"},
				{Path: "spec.sourceDir", From: "derive:artifacts.imageOutputDir", Semantic: "image-output-dir", Required: true},
				{Path: "spec.runtime", From: "override:runtime"},
				{Path: "spec.runtime", From: "const:ctr", Required: true},
			},
		}},
