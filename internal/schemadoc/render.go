...
func RenderGroupPage(in PageInput) []byte {
	...
	if len(in.TypicalFlows) > 0 {
		buf.WriteString("\n## Typical Flows\n")
		for _, flow := range in.TypicalFlows {
			buf.WriteString("\n### " + flow.Title + "\n\n")
			buf.WriteString("- kinds: `" + strings.Join(flow.Kinds, "`, `") + "`\n")
...
func RenderTypedStepsPage(inputs []PageInput) []byte {
	...
	buf.WriteString("Use these pages when you are deciding which kind to author. For top-level workflow and component document contracts, see [Workflow Model](workflow-model.md#workflow-schema-contract) and [Workspace Layout](workspace-layout.md#component-fragment-contract).\n\n")
