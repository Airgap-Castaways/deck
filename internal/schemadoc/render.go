...
func RenderGroupPage(in PageInput) []byte {
	return renderTypedStepGroupPage(in, PhaseInput{}, "../workflow-model.md#step-envelope-contract", "../typed-steps.md", nil)
}

func RenderTypedStepPhaseGroupPage(in PhasePage) []byte {
	return renderTypedStepGroupPage(in.Page, in.Phase, "../../workflow-model.md#step-envelope-contract", "../../typed-steps.md", in.PhaseGroups)
}

func renderTypedStepGroupPage(in PageInput, phase PhaseInput, workflowModelLink string, typedStepsLink string, groupsInPhase map[string]bool) []byte {
	var buf bytes.Buffer
	buf.WriteString("<!-- Code generated by make generate; DO NOT EDIT. -->\n")
	title := firstNonEmpty(in.Title, in.Group)
	summary := firstNonEmpty(in.Summary, in.Description, "Reference for typed workflow steps in this group.")
	buf.WriteString("# " + title + "\n\n")
	if strings.TrimSpace(phase.Title) != "" {
		buf.WriteString("Phase: " + phase.Title + "\n\n")
	}
	buf.WriteString(summary + "\n\n")
	buf.WriteString("## Summary\n\n")
	if strings.TrimSpace(phase.Key) != "" {
		buf.WriteString("- phase: `" + phase.Key + "`\n")
	}
	buf.WriteString("- group: `" + in.Group + "`\n")
	if len(in.Variants) > 0 {
		buf.WriteString("- kinds: " + strings.Join(localKindLinks(pageKinds(in.Variants)), ", ") + "\n")
	}
	if strings.TrimSpace(in.WhenToUse) != "" {
		buf.WriteString("\n## When To Use This Group\n\n")
		buf.WriteString(in.WhenToUse + "\n")
	}
	if len(in.TypicalFlows) > 0 {
		buf.WriteString("\n## Typical Flows\n")
		for _, flow := range filterFlows(in.TypicalFlows, pageKinds(in.Variants)) {
			buf.WriteString("\n### " + flow.Title + "\n\n")
			buf.WriteString("- kinds: " + strings.Join(localKindLinks(flow.Kinds), ", ") + "\n")
			if strings.TrimSpace(flow.Note) != "" {
				buf.WriteString("- note: " + flow.Note + "\n")
			}
		}
	}
	buf.WriteString("\n## Shared Step Fields\n\n")
	buf.WriteString("Shared step envelope fields such as `id`, `apiVersion`, `kind`, `when`, `parallelGroup`, `retry`, `timeout`, `register`, and `metadata` are documented in [Step Envelope Contract](" + workflowModelLink + ").\n")
	if len(in.Variants) > 1 {
		buf.WriteString("\n## Supported Kinds\n\n")
		for _, variant := range sortedVariants(in.Variants) {
			buf.WriteString("- [" + variant.Kind + "](#" + kindAnchor(variant.Kind) + "): " + firstNonEmpty(variant.Meta.Summary, variant.Description) + "\n")
		}
	}
	for _, variant := range sortedVariants(in.Variants) {
	...
	buf.WriteString("\n## Related\n\n")
	for _, group := range in.SeeAlso {
		if groupsInPhase != nil && !groupsInPhase[group] {
			continue
		}
		meta := MustGroupMeta(group)
		buf.WriteString("- [" + meta.Title + "](" + meta.Key + ".md)\n")
	}
	buf.WriteString("- [Typed Steps](" + typedStepsLink + ")\n")
	buf.WriteString("- [Step Envelope Contract](" + workflowModelLink + ")\n")
	return buf.Bytes()
}
...
func RenderTypedStepsPage(inputs []PageInput) []byte {
	var buf bytes.Buffer
	buf.WriteString("<!-- Code generated by make generate; DO NOT EDIT. -->\n")
	buf.WriteString("# Typed Steps\n\n")
	buf.WriteString("Choose typed workflow steps by workflow phase and task-oriented group.\n\n")
	...
	phasePages := PhasePages(inputs)
	for _, phase := range TypedStepPhases() {
		pages := phasePagesForPhase(phasePages, phase.Key)
		if len(pages) == 0 {
			continue
		}
		buf.WriteString("## " + phase.Title + "\n\n")
		buf.WriteString(phase.Description + "\n\n")
		buf.WriteString("| Group | Step Kinds | When to use |\n")
		buf.WriteString("|---|---|---|\n")
		for _, page := range pages {
			path := typedStepPhaseGroupPath(page.Phase.Key, page.Page.PageSlug)
			buf.WriteString("| [" + page.Page.Title + "](" + path + ") | ")
			buf.WriteString(strings.Join(kindLinks(pageKinds(page.Page.Variants), path), ", "))
			buf.WriteString(" | " + escapeMarkdownTableCell(page.Page.WhenToUse) + " |\n")
		}
		buf.WriteString("\n")
	}
	return buf.Bytes()
...
func TypedStepPhases() []PhaseInput {
	return []PhaseInput{
...
func PhasePages(inputs []PageInput) []PhasePage {
	pages := make([]PhasePage, 0)
	groupsByPhase := map[string]map[string]bool{}
	for _, phase := range TypedStepPhases() {
		for _, page := range inputs {
			variants := phaseFilteredVariants(page.Variants, phase.Key)
			if len(variants) == 0 {
				continue
			}
			phasePage := page
			phasePage.Variants = variants
			phasePage.TypicalFlows = filterFlows(page.TypicalFlows, pageKinds(variants))
			if groupsByPhase[phase.Key] == nil {
				groupsByPhase[phase.Key] = map[string]bool{}
			}
			groupsByPhase[phase.Key][phasePage.Group] = true
			pages = append(pages, PhasePage{Phase: phase, Page: phasePage})
		}
	}
	for i := range pages {
		pages[i].PhaseGroups = groupsByPhase[pages[i].Phase.Key]
	}
	return pages
}
...
func phaseFilteredVariants(variants []VariantInput, phase string) []VariantInput {
	out := make([]VariantInput, 0, len(variants))
...
func phasePagesForPhase(pages []PhasePage, phase string) []PhasePage {
	out := make([]PhasePage, 0)
	for _, page := range pages {
...
func typedStepPhaseGroupPath(phase string, slug string) string {
	return "typed-steps/" + phase + "/" + slug + ".md"
}

func kindLinks(kinds []string, pagePath string) []string {
	links := make([]string, 0, len(kinds))
	for _, kind := range kinds {
		links = append(links, "["+kind+"]("+pagePath+"#"+kindAnchor(kind)+")")
	}
	return links
}

func localKindLinks(kinds []string) []string {
	links := make([]string, 0, len(kinds))
	for _, kind := range kinds {
		links = append(links, "["+kind+"](#"+kindAnchor(kind)+")")
	}
	return links
}
...
func filterFlows(flows []GroupFlow, kinds []string) []GroupFlow {
	if len(flows) == 0 || len(kinds) == 0 {
		return nil
	}
	...
	out := make([]GroupFlow, 0, len(flows))
	for _, flow := range flows {
		filteredKinds := make([]string, 0, len(flow.Kinds))
		for _, kind := range flow.Kinds {
			if keepKinds[kind] {
