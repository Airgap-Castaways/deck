...
var groupMetadata = map[string]GroupMetadata{
	...
	"artifact-staging": {
		...
		Order:        20,
		TypicalFlows: []GroupFlow{{Title: "Stage offline packages", Kinds: []string{"DownloadPackage"}, Note: "Use Package Management during apply to consume the staged repository output."}, {Title: "Stage offline images", Kinds: []string{"DownloadImage"}, Note: "Use Kubernetes Lifecycle during apply to load staged image archives."}, {Title: "Fetch bundle files", Kinds: []string{"DownloadFile"}}},
	...
	"runtime-services": {
		...
		WhenToUse:    "Use this group when runtime settings or service state changes must take effect locally during apply.",
		Order:        50,
		TypicalFlows: []GroupFlow{{Title: "Configure containerd", Kinds: []string{"WriteContainerdConfig", "WriteContainerdRegistryHosts", "ManageService"}}, {Title: "Install or override a unit", Kinds: []string{"WriteSystemdUnit", "ManageService"}}},
