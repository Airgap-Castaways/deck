...
var toolMetadata = map[string]ToolMetadata{
	...
	"Packages": {
		...
		FieldDocs: map[string]FieldDoc{
			...
			"spec.restrictToRepos":       {Description: "Limit package manager visibility to these repository selectors during the operation. For apt, use repo file paths or globs; for dnf, use repo IDs. Prevents accidental pulls from other configured repos.", Example: "[offline-kubernetes]"},
			"spec.excludeRepos":          {Description: "Repository selectors to exclude from package resolution. For apt, selectors match repo file paths; for dnf, they match repo IDs.", Example: "[updates]"},
