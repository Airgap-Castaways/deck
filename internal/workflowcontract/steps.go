...
func StepDefinitions() []StepDefinition {
	defs := []StepDefinition{
		stepDef("Artifact", "artifact", "Artifact", "artifact", 10, "artifact.schema.json", "artifact", "public", "apply", "Install or extract pre-collected artifacts on the node.", "Use this when your apply workflow needs binaries or archives that were prepared into the bundle.", []string{"apply"}, nil),
		stepDef("HostCheck", "host-check", "HostCheck", "host-check", 10, "host-check.schema.json", "host-check", "public", "prepare", "Run host checks before prepare execution.", "Use this at the start of prepare workflows to fail early on unsupported hosts.", []string{"prepare"}, []string{"passed", "failedChecks"}),
		...
		stepDef("PackageDownload", "package", "Package", "package", 10, "package.download.schema.json", "package.download", "public", "packages", "Download packages into the bundle.", "Use this during prepare to collect package-manager content for offline installation.", []string{"prepare"}, []string{"artifacts"}),
		stepDef("PackageInstall", "package", "Package", "package", 20, "package.install.schema.json", "package.install", "public", "packages", "Install packages on the local node.", "Use this during apply to install packages from configured local or mirrored repositories.", []string{"apply"}, nil),
		stepDef("RepositoryConfigure", "repository", "Repository", "repository", 10, "repository.configure.schema.json", "repository.configure", "public", "packages", "Configure apt or yum repository definitions.", "Use this before refreshing caches or installing packages from a local mirror.", []string{"apply"}, []string{"path"}),
		stepDef("RepositoryRefresh", "repository", "Repository", "repository", 20, "repository.refresh.schema.json", "repository.refresh", "public", "packages", "Refresh package metadata with repo filtering.", "Use this after writing repo definitions and before package install steps that depend on fresh metadata.", []string{"apply"}, nil),
		...
		stepDef("WaitCommand", "wait", "Wait", "wait", 20, "wait.command.schema.json", "wait.command", "public", "control-flow", "Wait until a command exits successfully.", "Use this when a dependent step should wait for a local command-based condition to succeed.", []string{"apply"}, nil),
