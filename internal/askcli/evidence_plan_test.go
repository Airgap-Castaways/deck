...
func TestBuildEvidencePlanMarksVersionedUnknownInstallRequestRequired(t *testing.T) {
	plan := buildHeuristicEvidencePlan("Install AcmeMesh 1.35.1 on Ubuntu 22.04 and list prerequisites", askretrieve.WorkspaceSummary{}, askintent.Decision{Route: askintent.RouteQuestion})
...
func TestBuildEvidencePlanTreatsWorkspaceReviewAsUnnecessary(t *testing.T) {
	plan := buildHeuristicEvidencePlan("Review workflows/scenarios/apply.yaml in this workspace", askretrieve.WorkspaceSummary{HasWorkflowTree: true}, askintent.Decision{Route: askintent.RouteReview})
...
func TestBuildEvidencePlanTreatsInternalStepspecExplainAsLocal(t *testing.T) {
	plan := buildHeuristicEvidencePlan("Explain the typed step builders defined in internal/stepspec for DownloadPackage, InstallPackage, InitKubeadm, and JoinKubeadm.", askretrieve.WorkspaceSummary{HasWorkflowTree: true}, askintent.Decision{Route: askintent.RouteExplain, Target: askintent.Target{Kind: "component", Path: "internal/stepspec"}})
	if plan.Decision != "unnecessary" {
...
func TestShouldUseLLMEvidencePlannerWhenExternalNeedLacksEntity(t *testing.T) {
	plan := buildHeuristicEvidencePlan("Install the latest release on Debian 12", askretrieve.WorkspaceSummary{}, askintent.Decision{Route: askintent.RouteQuestion})
...
func TestBuildEvidencePlanSkipsStopwordsWithoutAbortingEntityExtraction(t *testing.T) {
	plan := buildHeuristicEvidencePlan("Install the AcmeMesh on Debian 12", askretrieve.WorkspaceSummary{}, askintent.Decision{Route: askintent.RouteQuestion})
	found := false
