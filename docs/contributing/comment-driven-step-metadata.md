...
Public workflow step kinds in `deck` use a comment-driven source-of-truth model.
...
Public step kind docs are generated as one page per `kind` and indexed by workflow phase and group. Internal family metadata still matters for registration and schema generation, but public docs should not require operators to understand family names.
