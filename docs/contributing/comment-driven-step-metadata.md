...
For typed step taxonomy, keep these roles distinct:

- `kind`: concrete public step identity such as `WriteFile` or `CheckKubernetesCluster`
- `group`: public task taxonomy used to generate phase-scoped pages under `docs/reference/typed-steps/<phase>/*.md`
...
Public typed step docs are generated by workflow phase and group. Internal family metadata still matters for registration and schema generation, but public docs should not require operators to understand family names.
