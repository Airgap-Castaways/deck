...
- **Pre-flight validation catches mistakes early.** `deck lint` checks the workflow structure and every step schema before you leave the connected environment. Discovering a typo or a missing field inside the air gap is expensive.
- **The bundle is self-contained and verifiable.** Workflow definitions, all required artifacts, and the `deck` binary ship together in one archive. There are no surprise missing dependencies when you unpack on site.
