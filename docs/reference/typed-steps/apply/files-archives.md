...
- kinds: [EnsureDirectory](#ensuredirectory), [WriteFile](#writefile), [CopyFile](#copyfile), [ExtractArchive](#extractarchive), [EditFile](#editfile), [EditTOML](#edittoml), [EditYAML](#edityaml), [EditJSON](#editjson), [CreateSymlink](#createsymlink)
...
- kinds: [EnsureDirectory](#ensuredirectory), [WriteFile](#writefile), [CopyFile](#copyfile), [EditYAML](#edityaml)

### Expand prepared content

- kinds: [ExtractArchive](#extractarchive), [CreateSymlink](#createsymlink)
...
- [EnsureDirectory](#ensuredirectory): Ensure a directory exists with an optional mode.
- [WriteFile](#writefile): Write inline or templated file content to a destination path.
- [CopyFile](#copyfile): Copy a file from a declared source to a destination path.
- [ExtractArchive](#extractarchive): Extract an archive from a declared source into a destination directory.
- [EditFile](#editfile): Edit an existing file in place using ordered match rules.
- [EditTOML](#edittoml): Edit a TOML file in place using structured path operations.
- [EditYAML](#edityaml): Edit a YAML file in place using structured path operations.
- [EditJSON](#editjson): Edit a JSON file in place using structured path operations.
- [CreateSymlink](#createsymlink): Create or replace a symbolic link.
