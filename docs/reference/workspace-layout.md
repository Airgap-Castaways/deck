...
A `deck` workspace is organized into three main functional areas: **Workflows**, **Artifacts**, and **Metadata**.
...
- Each file here must be a complete workflow with `role`, `version`, and either `steps`, `phases`, or `artifacts`.
...
## Artifacts (`files/`, `packages/`, `images/`)
