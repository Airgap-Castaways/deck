...
It also replaces large procedural scripts with typed workflow steps. A typed `Packages` step immediately communicates its intent, whereas a shell block full of `yum` or `apt` commands hides the goal behind syntax.
