...
source_hash: a5c688e09464139cc204970e1e56d67f6e466618
---
# 오프라인 Kubernetes 튜토리얼

이 튜토리얼은 `deck`이 설계 목적에 맞는 환경, 즉 인터넷이 없고 SSH 기반 오케스트레이션이 없으며 각 노드에 로컬 운영자가 있는 환경에서 Kubernetes 유지보수 세션에 어떻게 들어맞는지 보여줍니다.

Kubernetes 호스트 준비 및 부트스트랩 절차는 빠르게 커집니다. 그 규모에서는 순수 셸 스크립트를 검토하기 어려워집니다. `deck`은 절차를 구조화된 상태로 유지하고, 전송 전에 검증하며, 노드가 필요로 하는 모든 것을 단일 아카이브로 번들합니다.

## 목표

연결된 환경에서 휴대 가능한 번들을 빌드하고, 이를 에어갭(망분리) 환경으로 옮긴 다음, 각 대상 노드에서 Kubernetes 지향 워크플로를 로컬로 실행합니다.

## 워크스페이스

이 튜토리얼의 참조 워크스페이스는 `examples/offline-kubernetes/`입니다. 여기에는 준비 워크플로, 부트스트랩·조인·리셋용 시나리오 워크플로, 그리고 시나리오가 임포트하는 재사용 가능한 컴포넌트 프래그먼트가 포함되어 있습니다.
...
- Debian 패키지(conntrack, socat, iptables 및 기타 호스트 사전 요구사항)
...
준비된 모든 아티팩트를 휴대 가능한 아카이브로 패키징합니다:
...
## 3. 번들을 에어갭(망분리) 사이트로 전송

`bundle.tar`을 해당 환경에서 승인된 경로(이동식 미디어, 통제된 게이트웨이, 또는 다른 사이트 승인 전달 방식)를 통해 옮깁니다. 적용을 실행하기 전에 대상 측에서 압축을 해제하세요.

## 4. 컨트롤플레인 노드 부트스트랩

컨트롤플레인 노드(예: `192.0.2.10`의 `cp-1`)에서 번들의 압축을 해제하고 부트스트랩 시나리오를 실행합니다:
...
`BEGIN`과 `END` 마커 사이의 텍스트(단일 줄 base64)를 복사하세요. 평문 조인 자격 증명은 로그, 적용 상태, 또는 제공되는 어떤 파일에도 기록되지 않습니다.

### Calico CNI

Calico 이미지는 준비되어 번들의 이미지 저장소에서 사용할 수 있습니다. CNI 매니페스트(Tigera operator 또는 `calico.yaml`)는 부트스트랩 후 **대역 외(out-of-band)** 로 적용됩니다 — 예를 들어 매니페스트를 번들에 배치하고 `kubeconfig`가 사용 가능해지면 클러스터 접근이 가능한 터미널에서 `kubectl apply -f`를 실행하는 방식입니다.

## 5. 워커 노드 조인

각 워커 노드(예: `worker-1`)에서 번들의 압축을 해제하고 조인 시나리오를 실행합니다:
...
1. **암호문(Ciphertext)** — 암호화된 조인 블록을 단일 줄로 붙여넣습니다(줄바꿈 없이).
2. **패스프레이즈(Passphrase)** — 부트스트랩 시 선택한 패스프레이즈로, 시크릿으로 입력됩니다(화면에 표시되지 않음).

워커는 조인 명령을 로컬에서 복호화하여 `JoinKubeadm`이 사용하는 임시 파일에 기록합니다. 평문은 네트워크를 통과하지 않으며 적용 상태나 로그에 저장되지 않습니다.

조인 단계 후 verify 단계가 해당 노드가 클러스터에 조인했는지 확인합니다.
...
일부 사이트에서는 에어갭 환경 내부에 임시 공유 번들 소스를 두어 여러 노드가 개별 전송 없이 동일한 릴리스를 가져올 수 있도록 하는 것이 유리합니다:
...
vars 기본값(`server.url: 192.0.2.10:5000`)이 이 패턴과 일치합니다. 이 선택은 명시적이고 부차적인 것으로 유지하세요. 핵심 워크플로는 각 노드에서의 로컬 `deck` 실행을 중심으로 합니다.
...
리셋 시나리오는 `kubeadm reset`을 실행하고, CRI를 중지하며, 네트워크 상태를 초기화하고, deck 아티팩트를 제거합니다. 마지막에는 운영자 대상 범위 고지를 표시합니다.

### 리셋 범위

리셋은 다음을 수행하지 **않습니다**:

- 클러스터 API에서 Node 객체를 제거하지 않습니다. 노드를 드레인하고 삭제하려면 컨트롤플레인 노드에서 실행하세요:
  ```bash
  kubectl drain <node> --ignore-daemonsets --delete-emptydir-data
  kubectl delete node <node>
  ```
- 컨테이너 런타임이나 Kubernetes 바이너리를 제거하지 않습니다. 노드가 빠르게 다시 조인할 수 있도록 유지됩니다. 전체 제거는 수동으로 수행합니다:
  ```bash
  systemctl disable --now containerd-k8s
  rm -rf /opt/containerd-k8s /data/containerd-k8s /etc/containerd-k8s
  ```
- swap를 다시 활성화하거나, 오프라인 저장소를 제거하거나, OS 패키지를 제거하지 않습니다.
