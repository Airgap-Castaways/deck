...
source_hash: 2c1586192cf02b03052448d961e885d75492914e
---
# 진단: 에러 코드

deck는 CLI 에러를 `CODE: message` 형식으로 렌더링합니다(`internal/errcode` 참고). 아래 코드는 스크립트와 로그에서 매칭할 수 있는 안정적이고 기계 판독 가능한 식별자입니다.

> **컨트리뷰터:** 소스에 새로운 `E_*` 코드를 추가할 때는 아래 해당 섹션에 행을 추가하세요. `internal/doccheck` 드리프트 테스트는 코드가 문서화될 때까지 빌드를 실패시킵니다.

## 번들

| Code | 의미 | 컨텍스트 |
|------|---------|---------|
| `E_MANIFEST_MISSING` | `.deck/manifest.json`(또는 tar 아카이브의 매니페스트 항목)을 찾을 수 없음 | 적용 시작 시 번들 무결성 검사; `bundle verify`; 번들 병합 |
| `E_MANIFEST_EMPTY` | 번들 매니페스트가 존재하지만 추적되는 항목이 없음 | `bundle verify`; 적용 시작 시 매니페스트 검증 |
| `E_BUNDLE_INTEGRITY` | 번들 아티팩트가 누락되었거나, 크기 또는 SHA-256 불일치가 있거나, 매니페스트 항목이 구조적으로 유효하지 않음 | `bundle verify`, 적용 시작 시 검증, 번들 병합 |
| `E_BUNDLE_IMPORT_PATH_TRAVERSAL` | 번들 아카이브의 tar 항목에 대상 루트를 벗어나는 경로(예: `../`)가 포함됨 | `bundle import` 아카이브 추출 |
| `E_BUNDLE_IMPORT_INVALID_PREFIX` | tar 항목 경로가 필수 `bundle/` 접두사로 시작하지 않음 | `bundle import` 아카이브 추출 |
| `E_BUNDLE_IMPORT_UNSUPPORTED_TYPE` | tar 항목이 지원되지 않는 파일 유형(일반 파일 또는 디렉터리가 아님)을 가짐 | `bundle import` 아카이브 추출 |

## 워크플로 검증

| Code | 의미 | 컨텍스트 |
|------|---------|---------|
| `E_SCHEMA_INVALID` | 워크플로 스텝 또는 단계에 유효하지 않거나 사용 중단된 필드 값이 있음 | 적용/준비 전 스키마 검증; `deck validate` |
...
| `E_DUPLICATE_STEP_ID` | 둘 이상의 워크플로 스텝이 동일한 `id`를 공유함 | 워크플로 시맨틱 검증 |
| `E_RUNTIME_VAR_RESERVED` | `register`에서 사용된 변수 이름이 내장 런타임 변수와 충돌함 | register 출력의 시맨틱 검증; 런타임 변수 등록 |
| `E_REGISTER_VAR_INVALID` | 스텝의 `register` 변수 이름이 유효하지 않음(필수 패턴과 일치하지 않음) | 워크플로 시맨틱 검증 |
| `E_RUNTIME_VAR_REDEFINED` | `runtime.*` 변수가 워크플로 스텝 전체에서 두 번 이상 정의됨 | 워크플로 시맨틱 검증 |
| `E_TEMPLATE_SINGLE_BRACE` | 템플릿 표현식이 필수 이중 중괄호 구문(`{{var}}`) 대신 단일 중괄호 구문(`{var}`)을 사용함 | 스텝 검증 중 템플릿 구문 검사 |
| `E_REGISTER_OUTPUT_NOT_FOUND` | `register` 블록이 해당 스텝이 생성하지 않는 출력 키를 참조함 | 시맨틱 검증; 런타임 스텝 출력 등록 |
| `E_CONDITION_EVAL` | `when` CEL 조건이 런타임에 평가에 실패함 | install 및 prepare 러너에서 스텝 조건 평가 |
...
| `E_PARALLEL_OUTPUT_CONFLICT` | 동일한 `parallelGroup`의 두 스텝이 모두 동일한 `register` 출력 키를 씀 | 병렬 그룹 충돌 감지 |
| `E_PARALLEL_RUNTIME_DEPENDENCY` | 한 스텝이 동일한 `parallelGroup`의 다른 스텝이 생성하는 `runtime.*` 변수를 읽음 | 병렬 그룹 의존성 검사 |
...
| Code | 의미 | 컨텍스트 |
|------|---------|---------|
| `E_PREPARE_KIND_UNSUPPORTED` | 준비 단계에서 지원되지 않는 스텝 종류 | prepare 러너의 스텝 디스패치 |
| `E_PREPARE_NO_ARTIFACTS` | 패키지 다운로드 스텝이 출력 디렉터리에 아티팩트를 생성하지 않음 | 준비 단계의 패키지 아티팩트 수집 |
| `E_PREPARE_SOURCE_NOT_FOUND` | 스텝에서 참조한 `source.path`를 구성된 fetch 소스 중 하나로 확인할 수 없음 | 준비 단계의 파일 및 아티팩트 다운로드 |
| `E_PREPARE_CHECKSUM_MISMATCH` | 다운로드된 아티팩트 체크섬이 예상 값과 일치하지 않음 | 준비 단계의 파일 다운로드 검증 |
| `E_PREPARE_OFFLINE_POLICY_BLOCK` | `source.url` 다운로드가 오프라인 정책에 의해 차단됨 | 오프라인 모드가 적용될 때 준비 단계의 파일 다운로드 |
| `E_PREPARE_RUNTIME_NOT_FOUND` | 패키지 준비에 사용할 수 있는 지원되는 컨테이너 런타임(docker/podman)이 없음 | 준비 단계의 컨테이너 런타임 감지 |
...
| Code | 의미 | 컨텍스트 |
|------|---------|---------|
| `E_INSTALL_KIND_UNSUPPORTED` | 설치(적용) 단계에서 지원되지 않는 스텝 종류 | install 러너의 스텝 디스패치 |
| `E_INSTALL_SOURCE_NOT_FOUND` | 필수 소스 아티팩트를 번들에서 찾을 수 없음 | install 스텝 실행 중 아티팩트 조회 |
| `E_INSTALL_CHECKSUM_MISMATCH` | 아티팩트의 체크섬이 예상 값과 일치하지 않음 | 설치 중 아티팩트 무결성 검증 |
| `E_INSTALL_OFFLINE_POLICY_BLOCK` | 설치 중 네트워크 다운로드가 오프라인 정책에 의해 차단됨 | 설치 중 네트워크 액세스 게이트 |
| `E_INSTALL_CHECKHOST_FAILED` | 설치 단계 중 `CheckHost` 스텝이 실패함 | 설치 단계의 호스트 사전 검사 실행 |
| `E_INSTALL_CLUSTER_CHECK_FAILED` | `CheckKubernetesCluster` 스텝이 실패함 | 설치 중 Kubernetes 클러스터 준비 상태 검사 |
| `E_INSTALL_INTERACTION_FAILED` | 오퍼레이터 상호작용 스텝(prompt/input)이 실패함 | `Input`, `Confirm` 스텝 실행 |
| `E_INSTALL_INTERACTION_UNSUPPORTED` | 상호작용 스텝이 메시지에 시크릿 런타임 값을 렌더링하려고 시도함 | 오퍼레이터 상호작용 메시지 렌더링 |
...
| `E_INSTALL_INSTALLFILE_CONTENT_REQUIRED` | `InstallFile` 스텝에 지정된 콘텐츠가 없음 | `InstallFile` 스텝 검증 |
...
| `E_INSTALL_SYSTEMD_UNIT_SERVICE_NAME_REQUIRED` | `WriteSystemdUnit` 스텝이 서비스 이름을 필요로 하지만 제공되지 않음 | `WriteSystemdUnit` 스텝 검증 |
...
| Code | 의미 | 컨텍스트 |
|------|---------|---------|
| `E_INSTALL_WAIT_TIMEOUT` | 대기 스텝(WaitForFile, WaitForService 등)이 조건이 충족되기 전에 타임아웃됨 | 설치 중 모든 대기 스텝 종류 |
| `E_INSTALL_WAITPATH_PATH_REQUIRED` | 경로 기반 대기 스텝에 `path` 필드가 없음 | `WaitForFile`/`WaitForMissingFile` 스텝 검증 |
| `E_INSTALL_WAITPATH_STATE_INVALID` | 경로 기반 대기 스텝이 유효하지 않은 예상 상태를 지정함 | `WaitForFile`/`WaitForMissingFile` 스텝 검증 |
| `E_INSTALL_WAITPATH_TYPE_INVALID` | 경로 기반 대기 스텝이 유효하지 않은 경로 유형(`file`, `dir`, `any`가 아님)을 지정함 | `WaitForFile` 스텝 검증 |
| `E_INSTALL_WAITPATH_POLL_INTERVAL_INVALID` | 경로 기반 대기 스텝이 유효하지 않은 폴링 간격을 지정함 | `WaitForFile`/`WaitForMissingFile` 스텝 검증 |
| `E_INSTALL_WAITPATH_TIMEOUT` | 경로 기반 대기 스텝이 타임아웃됨 | `WaitForFile`/`WaitForMissingFile` 스텝 실행 |
...
| `E_INSTALL_VERIFY_IMAGES_NOT_FOUND` | 하나 이상의 필수 이미지가 호스트에 존재하지 않음 | `VerifyImage` 스텝 실행 |
...
| `E_INSTALL_ARTIFACT_ARCH_UNSUPPORTED` | 번들의 아티팩트 중 호스트 아키텍처와 일치하는 것이 없음 | 아티팩트 아키텍처 선택 |
| `E_INSTALL_ARTIFACT_SOURCE_INVALID` | 아티팩트 소스 명세가 유효하지 않음 | 설치 중 아티팩트 소스 검증 |

## 설치 — Kubernetes (kubeadm)

| Code | 의미 | 컨텍스트 |
|------|---------|---------|
| `E_INSTALL_KUBEADM_INIT_MODE_INVALID` | `InitKubeadm` 스텝이 유효하지 않은 모드를 지정함 | `InitKubeadm` 스텝 검증 |
| `E_INSTALL_KUBEADM_INIT_JOINFILE_REQUIRED` | `InitKubeadm` 스텝이 join-file 경로를 필요로 하지만 제공되지 않음 | `InitKubeadm` 스텝 검증 |
...
| `E_INSTALL_KUBEADM_JOIN_JOINFILE_REQUIRED` | `JoinKubeadm` 스텝이 join-file 경로를 필요로 하지만 제공되지 않음 | `JoinKubeadm` 스텝 검증 |
| `E_INSTALL_KUBEADM_JOIN_FILE_NOT_FOUND` | `JoinKubeadm` 스텝이 참조한 join 구성 파일이 존재하지 않음 | `JoinKubeadm` 스텝 실행 |
| `E_INSTALL_KUBEADM_JOIN_INPUT_CONFLICT` | `JoinKubeadm` 스텝이 충돌하는 join 입력(예: join 명령과 join 파일 모두)을 지정함 | `JoinKubeadm` 스텝 검증 |
| `E_INSTALL_KUBEADM_JOIN_COMMAND_MISSING` | `JoinKubeadm` 스텝이 join 명령을 필요로 하지만 찾을 수 없음 | `JoinKubeadm` 스텝 검증 |
...
| Code | 의미 | 컨텍스트 |
|------|---------|---------|
| `E_SERVER_TLS_PARTIAL_FILES` | TLS 인증서 파일과 키 파일 중 정확히 하나만 존재함(둘 다 함께 제공하거나 둘 다 제공하지 않아야 함) | 서버 시작 시 TLS 구성 |
