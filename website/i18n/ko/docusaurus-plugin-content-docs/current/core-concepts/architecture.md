...
`deck`은 에어갭(망분리) 및 운영상 제약이 있는 환경을 위한 로컬 우선(local-first) 워크플로 도구입니다. 설계는 단순한 전제에서 출발합니다: 연결되어 있는 동안 필요한 것을 수집하고, 명시적이고 검증 가능한 번들로 경계를 넘어 운반하며, 변경이 필요한 머신에서 로컬로 실행합니다.
...
`deck`은 무인 조정(unattended reconciliation)이 아니라 매뉴얼 우선(manual-first) 운영을 중심으로 설계되었습니다.
...
이 경우에도 `deck`은 중앙 조정 컨트롤러로 동작하지 않습니다. 서버는 사이트 로컬 배포 헬퍼입니다: 에어갭 내부에서 번들 콘텐츠를 서빙하지만, 노드 실행을 대신하지는 않습니다. 각 노드는 여전히 로컬로 실행합니다.
...
`deck`은 셸 중심 절차보다 타입이 지정된 스텝을 선호합니다. 이는 단순한 문서상의 선호가 아니라 아키텍처적 선택입니다.
...
이 설계의 또 다른 이유는 사용자 혼란을 줄이는 것입니다. `deck`은 동일한 동작을 표현하는 여러 겹치는 방식 대신, 일반적인 운영 작업에 하나의 명확한 타입 지정 형태를 부여하려고 합니다. 목표는 모든 엣지 케이스를 즉시 모델링하는 것이 아닙니다. 목표는 운영자가 보통 어떤 스텝 종류를 사용해야 하는지, 그리고 그것이 어떤 동작을 함의하는지 예측할 수 있을 만큼 주 경로를 충분히 단순하게 유지하는 것입니다.
...
내부적으로 `deck`은 등록 및 코드 생성을 위해 여전히 패밀리 분류체계를 유지합니다. 이 분리는 의도적입니다:
...
`deck`은 명령 표면을 소수의 라이프사이클 단계를 중심으로 구성하려고 합니다:
...
`deck`은 단순함을 단순한 구현상의 선호가 아니라 사용성 기능으로 취급합니다.
...
`deck`은 Go 모델 정의를 워크플로 및 스텝 구조의 주된 진실 공급원(source of truth)으로 취급합니다.
...
`deck`은 아직 릴리스되지 않았으므로, 프로젝트는 현재 폐기된 모든 설계에 대해 레거시 호환성 레이어를 유지하기보다는 더 작은 표준 모델로 수렴하는 것을 선호합니다.
...
`deck`은 의도적으로 다음이 되는 것을 목표로 하지 않습니다:
